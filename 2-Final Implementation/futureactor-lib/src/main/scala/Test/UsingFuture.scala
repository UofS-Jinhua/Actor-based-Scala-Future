    ...
    val testingActor = system.actorOf(Props(new Actor {
        def receive = {
            case "Start" =>
                ...
                val futures: List[Future[Int]] = List.tabulate(1000)(i => Future(testing_f2(i, 1000.millis)))
                ...
                // val future1 = Future(testing_f2(1, 1.millis))
                ...
                val pw = new PrintWriter(new FileOutputStream(new File("fututre_log_1000_1000.txt"), true))
                pw.write(s"Execution time: ${end_time - start_time} ms, lenth: ${result.length}\n")
                pw.close()
