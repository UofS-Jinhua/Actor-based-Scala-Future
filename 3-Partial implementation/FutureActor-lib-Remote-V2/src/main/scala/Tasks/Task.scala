...
abstract class Task[T] extends Awaitable[T]{
     ...
     */
    def Start(op: SerializableFunction_1[T]): Task[T]
    ...
    //  */
    def Zip[U](task: RemoteTask[U]): Task[(T, U)]
    ...
    //   */
      def Group[T](tasks: Seq[RemoteTask[T]]): Task[List[T]]
     ...
     */
    def NextStep[U](op2: SerializableFunction_2[T, U]): Task[U]
     ...
     * @return the current `Task` instance
     */
    def RecoverWith(func: SerializableFunction_1[T]): Task[T]
     ...
     * @return the current `Task` instance
     */
    def RecoverWith(func: SerializableFunction_3[T]): Task[T]
